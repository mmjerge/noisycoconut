...
Now includes self-consistency baseline for comparison.
...
# Methods to run
RUN_SELF_CONSISTENCY = True  # Standard autoregressive baseline
RUN_NOISY_COCONUT = False     # Your noisy coconut method
...
def create_standard_input(tokenizer, question: str, benchmark: str = "gsm8k"):
    """Create input for standard autoregressive generation (self-consistency baseline).
    
    Uses the same prompting as Coconut but without the latent tokens.
    """
    if benchmark == "mmlu":
        instruction = f"{question}\n\nPlease solve this step by step. At the end, clearly state your final answer as 'The answer is X' where X is A, B, C, or D."
    else:
...
def test_question_with_self_consistency(
    base_model,
    tokenizer,
    question: str,
    num_samples: int,
    ...
    """
    Test a single question using standard self-consistency.
    
    This is the baseline method: standard autoregressive generation with
    temperature sampling, generating multiple samples and using majority voting.
    """
    base_model.eval()

    input_ids = create_standard_input(tokenizer, question, benchmark=benchmark)
    ...
    with torch.no_grad():
        try:
            ...
            for _ in range(num_samples):
                ...
                generated_ids = output_ids[0, original_len:]
                generated_text = tokenizer.decode(generated_ids, skip_special_tokens=True)
                generated_text = generated_text.strip()

                sample_answer = extract_answer(generated_text, benchmark)

                sample_texts.append(generated_text)
                sample_answers.append(sample_answer)

            majority_answer = majority_vote(sample_answers)
            vote_distribution = dict(Counter(sample_answers))
...
def run_quick_test():
    ...
    config = {
        ...
        "run_noisy_coconut": RUN_NOISY_COCONUT
    }
    ...
    print(f"Branches/Samples: {NUM_BRANCHES}")
    print(f"Noise scales: {NOISE_SCALES}")
    print(f"Noise type: {NOISE_TYPE}")
    print(f"Temperature: {TEMPERATURE}, Top-p: {TOP_P}")
    print(f"Device: {device}")
    print(f"Methods: Self-Consistency={RUN_SELF_CONSISTENCY}, Noisy Coconut={RUN_NOISY_COCONUT}")
    print("="*70)
    ...
    try:
        for q_idx, q_data in enumerate(questions_to_process):
            ...
            if RUN_SELF_CONSISTENCY:
                print(f"\n{'-'*70}")
                print(f"[SELF-CONSISTENCY] Standard autoregressive with {NUM_BRANCHES} samples...")
                print(f"{'-'*70}")

                sc_result = test_question_with_self_consistency(
                ...
                if sc_result["success"]:
                    ...
                    is_correct = sc_result["majority_answer"] == expected_answer

                    print(f"\n✓ Self-Consistency generation successful!")
                    print(f"\nSample answers: {sc_result['sample_answers']}")
                    print(f"Vote distribution: {sc_result['vote_distribution']}")
                    print(f"\nMajority vote: {sc_result['majority_answer']}")
                    print(f"Expected: {expected_answer}")
                    print(f"Result: {'✓ CORRECT' if is_correct else '✗ INCORRECT'}")

                    if sc_result['sample_texts']:
                        print(f"\nExample output (Sample 1):")
                        print(f"  {sc_result['sample_texts'][0][:200]}...")

                    question_results["self_consistency"] = {
                        "method": "self_consistency",
                        "sample_texts": sc_result["sample_texts"],
                        "sample_answers": sc_result["sample_answers"],
                        "majority_answer": sc_result["majority_answer"],
                    ...
                    print(f"\n✗ Self-Consistency Error: {sc_result['error']}")
                    question_results["self_consistency"] = {
                        "method": "self_consistency",
            ...
            # ================================================================
            # NOISY COCONUT METHOD
            # ================================================================
            if RUN_NOISY_COCONUT:
                for noise_scale in NOISE_SCALES:
                    print(f"\n{'-'*70}")
                    print(f"[NOISY COCONUT] noise={noise_scale} with {NUM_BRANCHES} branches...")
                    print(f"{'-'*70}")
                    ...
                    if result["success"]:
                        ...
                        print(f"\n✓ Noisy Coconut generation successful!")
                        ...
                        print(f"\n✗ Noisy Coconut Error: {result['error']}")
                        question_results["noise_tests"].append({
                            "method": "noisy_coconut",
                            "noise_scale": noise_scale,
                            "error": result["error"]
    ...
    sc_correct = 0
    sc_total = 0
    nc_correct = {ns: 0 for ns in NOISE_SCALES}
    nc_total = {ns: 0 for ns in NOISE_SCALES}
    ...
    for q_idx, q_result in enumerate(results):
        ...
        if q_result.get("self_consistency"):
            sc = q_result["self_consistency"]
            if "majority_answer" in sc:
                sc_total += 1
                if sc.get("is_correct"):
                    sc_correct += 1
                correct = "✓" if sc.get("is_correct") else "✗"
                print(f"  [Self-Consistency]: {correct} Majority={sc['majority_answer']}, Votes={sc['vote_distribution']}")
            else:
                print(f"  [Self-Consistency]: ✗ Error")

        # Noisy Coconut results
        for test in q_result.get("noise_tests", []):
            noise = test.get("noise_scale", "?")
            if "majority_answer" in test:
                nc_total[noise] = nc_total.get(noise, 0) + 1
                if test.get("is_correct"):
                    nc_correct[noise] = nc_correct.get(noise, 0) + 1
                majority = test["majority_answer"]
                correct = "✓" if test.get("is_correct") else "✗"
                print(f"  [Noisy Coconut noise={noise}]: {correct} Majority={majority}, Votes={test['vote_distribution']}")
            else:
                print(f"  [Noisy Coconut noise={noise}]: ✗ Error")
    ...
    if RUN_SELF_CONSISTENCY and sc_total > 0:
        sc_acc = sc_correct / sc_total * 100
        print(f"Self-Consistency:        {sc_correct}/{sc_total} = {sc_acc:.2f}%")
    
    if RUN_NOISY_COCONUT:
        for noise_scale in NOISE_SCALES:
            if nc_total.get(noise_scale, 0) > 0:
                nc_acc = nc_correct[noise_scale] / nc_total[noise_scale] * 100
                print(f"Noisy Coconut (σ={noise_scale}): {nc_correct[noise_scale]}/{nc_total[noise_scale]} = {nc_acc:.2f}%")
        ...
        json.dump({
            ...
            "aggregate": {
                "self_consistency": {
                    "correct": sc_correct,
                    "total": sc_total,
                    "accuracy": sc_correct / sc_total * 100 if sc_total > 0 else 0
                } if RUN_SELF_CONSISTENCY else None,
                "noisy_coconut": {
                    str(ns): {
                        "correct": nc_correct.get(ns, 0),
                        "total": nc_total.get(ns, 0),
                        "accuracy": nc_correct.get(ns, 0) / nc_total.get(ns, 1) * 100 if nc_total.get(ns, 0) > 0 else 0
                    } for ns in NOISE_SCALES
                } if RUN_NOISY_COCONUT else None
            }
        }, f, indent=2)
