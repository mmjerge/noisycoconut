...
Usage:
    python run_logits.py                          # Use default args/noisy-coconut.yaml
    python run_logits.py --config my.yaml         # Use custom config file
    python run_logits.py experiment.num_questions=50  # Override via CLI
"""
...
from noisycoconut import Coconut
...
def test_question_with_branching(
    ...
    with torch.no_grad():
        try:
            all_branches = coconut_model.generate_with_branching_logits(
                input_ids=input_ids,
            ...
            vote_distribution = agg["vote_counts"]
        ...
        for q_data in pbar:
            ...
            for noise_scale in noise_pbar:
                ...
                result = test_question_with_branching(
                    ...
                    noise_schedule=getattr(cfg.noise, "schedule", "none"),
                    lambda_decay=getattr(cfg.noise, "lambda_decay", 0.5),
                    noise_at_step=getattr(cfg.noise, "injection_step", 1)
                )
...
def main():
    # Parse --config argument if provided, otherwise use the default config.
    config_path = "args/noisy-coconut.yaml"
