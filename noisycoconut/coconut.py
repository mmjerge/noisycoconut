...
class Coconut(nn.Module):
    ...
    def generate_with_branching_logits(
        self,
        input_ids: torch.Tensor,
        attention_mask: torch.Tensor,
        max_new_tokens: int = 16,
        ...
        **kwargs
        ...
        """
        Branching generation that additionally returns per-token log probabilities.

        This mirrors :meth:`generate_with_branching` (same noise model, injection
        point, and per-step decay schedule) but, for each branch, also records the
        model log-probability of every token it sampled.  These log probs feed the
        logit probability-mass voting used by ``run_logits.py``.

        Returns a list of ``(generated_sequence_ids, per_token_log_probs)`` tuples,
        one per branch.  The log prob of a token is taken from the full
        ``log_softmax`` of the un-temperature-scaled logits (i.e. the model's true
        probability for the token that was actually sampled).
        """
        assert input_ids.shape[0] == 1, "only support batch_size == 1"
        assert 1 <= noise_at_step <= MAX_N_LATENT, f"noise_at_step must be between 1 and {MAX_N_LATENT}"
        device = input_ids.device
        ...
        print(f"\n=== Branching Generation w/ logits (N={num_branches}, noise={noise_scale} at step {noise_at_step}) ===")
        ...
        # Hidden state at each latent step (identical across branches before branching).
        shared_trajectory: List[torch.Tensor] = []
        ...
        for branch_idx, branch in enumerate(all_branches):
            ...
            # Sample first token (and record its log prob under the model distribution).
            raw_logits = outputs.logits[0, -1].to(torch.float64)
            full_log_probs = torch.log_softmax(raw_logits, dim=-1)
            ...
            probs = torch.softmax(logits, dim=-1)
            next_token = torch.multinomial(probs, num_samples=1).item()
            token_log_probs.append(full_log_probs[next_token].item())
        ...
        print(f"=== Completed {num_branches} branches (with logits) ===\n")
